...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByGdod = async () => {
    await axios.get(`http://localhost:8000/api/equipmentAndMaterialsPeriodicInspections/bygdod/${props.unitid}`)
      .then((response) => {
  ...
  const getUnitDetailsByHativa = async () => {
    await axios.get(`http://localhost:8000/api/equipmentAndMaterialsPeriodicInspections/byhativa/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByOgda = async () => {
    await axios.get(`http://localhost:8000/api/equipmentAndMaterialsPeriodicInspections/byogda/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByPikod = async () => {
    await axios.get(`http://localhost:8000/api/equipmentAndMaterialsPeriodicInspections/bypikod/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/equipmentAndMaterialsPeriodicInspectionsDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/equipmentAndMaterialsPeriodicInspections/${tempData}`).then((response) => {
        init();
      })
        .catch((error) => {
          console.log(error);
      ...
      <>
        <div style={{ float: 'right', paddingBottom: '5px' }}>
          <ReactHTMLTableToExcel
            ...
            className="btn-green"
            table="table-to-xls"
            filename="קובץ - בדיקות תקופתיות לציוד וחומרים"
            sheet="קובץ - בדיקות תקופתיות לציוד וחומרים"
                      ...
                      if (cell.column.id == "_id") {
                        return (
                          <td>
                            <a
                              href={
                                "http://localhost:8000/api/downloadFile?collec=equipmentAndMaterialsPeriodicInspections&id=" +
                                cell.value.toString()
                              }
                              target="_blank"
                            >
