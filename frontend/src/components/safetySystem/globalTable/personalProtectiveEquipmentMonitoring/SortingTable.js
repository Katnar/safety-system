...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByGdod = async () => {
    await axios.get(`http://localhost:8000/api/personalProtectiveEquipmentMonitoring/bygdod/${props.unitid}`)
      .then((response) => {
  ...
  const getUnitDetailsByHativa = async () => {
    await axios.get(`http://localhost:8000/api/personalProtectiveEquipmentMonitoring/byhativa/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByOgda = async () => {
    await axios.get(`http://localhost:8000/api/personalProtectiveEquipmentMonitoring/byogda/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByPikod = async () => {
    await axios.get(`http://localhost:8000/api/personalProtectiveEquipmentMonitoring/bypikod/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/personalProtectiveEquipmentMonitoringDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/personalProtectiveEquipmentMonitoring/${tempData}`).then((response) => {
        init();
      })
        .catch((error) => {
          console.log(error);
                ...
                return (
                  <tr {...row.getRowProps()}>
                    {row.cells.map((cell) => {
                      ...
                      if (cell.column.id == "mkt") {
                        return <td>{cell.value}</td>;
                      }
                      if (cell.column.id == "amount") {
                        return <td>{cell.value}</td>;
                      }
                      if (cell.column.id == "equipmentLocation") {
                        return <td>{cell.value}</td>;
                      }
                    ...
                    <td role="cell">
                      ...
                      >
                        {" "}
                        <Link
                          to={`/GlobalPersonalProtectiveEquipmentMonitoringForm/${row.original._id}`}
                        >
