...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByGdod = async () => {
    await axios.get(`http://localhost:8000/api/reviewsDocumentation/bygdod/${props.unitid}`)
      .then((response) => {
  ...
  const getUnitDetailsByHativa = async () => {
    await axios.get(`http://localhost:8000/api/reviewsDocumentation/byhativa/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const getUnitDetailsByOgda = async () => {
    await axios.get(`http://localhost:8000/api/reviewsDocumentation/byogda/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const getUnitDetailsByPikod = async () => {
    await axios.get(`http://localhost:8000/api/reviewsDocumentation/bypikod/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/reviewsDocumentationDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/reviewsDocumentation/${tempData}`).then((response) => {
        init();
      })
        .catch((error) => {
          console.log(error);
                  ...
                  <tr {...row.getRowProps()}>
                      ...
                      if (cell.column.id == "_id") {
                        return (
                          <td>
                            <a
                              href={
                                "http://localhost:8000/api/downloadFile?collec=reviewsDocumentation&id=" +
                                cell.value.toString()
                              }
                              target="_blank"
                            >
                    ...
                    <td role="cell">
                      ...
                      >
                        {" "}
                        <Link
                          to={`/GlobalReviewsDocumentationForm/${row.original._id}`}
                        >
