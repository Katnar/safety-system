...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByHativa = async () => {
    ...
    await axios.get(`http://localhost:8000/api/reviewsDocumentation`)
      .then((response) => {
                ...
                <tr {...row.getRowProps()}>
                  {row.cells.map((cell) => {
                    ...
                    if (cell.column.id == "_id") {
                      return <td><a href={"http://localhost:8000/api/downloadFile?collec=reviewsDocumentation&id="+cell.value.toString()} target="_blank"><FaFileDownload/></a></td>;
                    }
                  ...
                  <td role="cell">
                    ...
                    >
                      {" "}
                      <Link
                        to={`/reviewsDocumentationGdodForm/${row.original._id}`}
                      >
