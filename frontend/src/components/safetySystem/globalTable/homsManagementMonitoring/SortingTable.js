...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByGdod = async () => {
    await axios.get(`http://localhost:8000/api/homsManagementMonitoring/bygdod/${props.unitid}`)
      .then((response) => {
  ...
  const getUnitDetailsByHativa = async () => {
    await axios.get(`http://localhost:8000/api/homsManagementMonitoring/byhativa/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByOgda = async () => {
    await axios.get(`http://localhost:8000/api/homsManagementMonitoring/byogda/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByPikod = async () => {
    await axios.get(`http://localhost:8000/api/homsManagementMonitoring/bypikod/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/homsManagementMonitoringDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/homsManagementMonitoring/${tempData}`).then((response) => {
        init();
      })
        .catch((error) => {
          console.log(error);
      ...
      <>
        <div style={{ float: 'right', paddingBottom: '5px' }}>
          <ReactHTMLTableToExcel
            ...
            className="btn-green"
            table="table-to-xls"
            filename="קובץ - ניהול חומ''ס"
            sheet="קובץ - ניהול חומ''ס"
                  ...
                  <tr {...row.getRowProps()}>
                    {row.cells.map((cell) => {
                      ...
                      if (cell.column.id == "_id") {
                        return <td><a href={"http://localhost:8000/api/downloadFile?collec=homsManagementMonitoring&id=" + cell.value.toString()} target="_blank"><FaFileDownload /></a></td>;
                      }
