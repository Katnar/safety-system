...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByGdod = async () => {
    await axios.get(`http://localhost:8000/api/unitId/bygdod/${props.unitid}`)
      .then((response) => {
  ...
  const getUnitDetailsByHativa = async () => {
    await axios.get(`http://localhost:8000/api/unitId/byhativa/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByOgda = async () => {
    await axios.get(`http://localhost:8000/api/unitId/byogda/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByPikod = async () => {
    await axios.get(`http://localhost:8000/api/unitId/bypikod/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/unitIdDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/unitId/${tempData}`).then((response) => {
        init();
      })
        .catch((error) => {
          console.log(error);
      ...
      <>
        <div style={{ float: 'right', paddingBottom: '5px' }}>
          <ReactHTMLTableToExcel
            ...
            className="btn-green"
            table="table-to-xls"
            filename="קובץ - תעודת זהות יחידה"
            sheet="קובץ - תעודת זהות יחידה"
