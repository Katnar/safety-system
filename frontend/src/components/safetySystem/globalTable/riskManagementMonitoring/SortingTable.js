...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByGdod = async () => {
    await axios.get(`http://localhost:8000/api/riskManagementMonitoring/bygdod/${props.unitid}`)
      .then((response) => {
  ...
  const getUnitDetailsByHativa = async () => {
    await axios.get(`http://localhost:8000/api/riskManagementMonitoring/byhativa/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const getUnitDetailsByOgda = async () => {
    await axios.get(`http://localhost:8000/api/riskManagementMonitoring/byogda/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const getUnitDetailsByPikod = async () => {
    await axios.get(`http://localhost:8000/api/riskManagementMonitoring/bypikod/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/riskManagementMonitoringDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/riskManagementMonitoring/${tempData}`).then((response) => {
        init();
      })
        .catch((error) => {
          console.log(error);
      ...
      <>
        <div style={{ float: 'right', paddingBottom: '5px' }}>
          <ReactHTMLTableToExcel
            ...
            className="btn-green"
            table="table-to-xls"
            filename="קובץ - מעקב ניהול סיכונים"
            sheet="קובץ - מעקב ניהול סיכונים"
