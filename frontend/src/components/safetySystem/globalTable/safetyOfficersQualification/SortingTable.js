...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByGdod = async () => {
    await axios.get(`http://localhost:8000/api/safetyOfficersQualification/bygdod/${props.unitid}`)
      .then((response) => {
  ...
  const getUnitDetailsByHativa = async () => {
    await axios.get(`http://localhost:8000/api/safetyOfficersQualification/byhativa/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const getUnitDetailsByOgda = async () => {
    await axios.get(`http://localhost:8000/api/safetyOfficersQualification/byogda/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const getUnitDetailsByPikod = async () => {
    await axios.get(`http://localhost:8000/api/safetyOfficersQualification/bypikod/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/safetyOfficersQualificationDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/safetyOfficersQualification/${tempData}`).then((response) => {
        init();
      })
        .catch((error) => {
          console.log(error);
      ...
      <>
        <div style={{ float: 'right', paddingBottom: '5px' }}>
          <ReactHTMLTableToExcel
            ...
            className="btn-green"
            table="table-to-xls"
            filename="קובץ - כשירות ממונים על הבטיחות"
            sheet="קובץ - כשירות ממונים על הבטיחות"
                  ...
                  <tr {...row.getRowProps()}>
                    ...
                    <td role="cell">
                      ...
                      >
                        {" "}
                        <Link
                          to={`/GlobalSafetyOfficersQualificationForm/${row.original._id}`}
                        >
