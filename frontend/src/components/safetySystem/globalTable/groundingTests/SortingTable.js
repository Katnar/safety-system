...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByGdod = async () => {
    await axios.get(`http://localhost:8000/api/groundingTests/bygdod/${props.unitid}`)
      .then((response) => {
  ...
  const getUnitDetailsByHativa = async () => {
    await axios.get(`http://localhost:8000/api/groundingTests/byhativa/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByOgda = async () => {
    await axios.get(`http://localhost:8000/api/groundingTests/byogda/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const getUnitDetailsByPikod = async () => {
    await axios.get(`http://localhost:8000/api/groundingTests/bypikod/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
      })
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/groundingTestsDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/groundingTests/${tempData}`).then((response) => {
        init();
      })
        .catch((error) => {
          console.log(error);
      ...
      <>
        <div style={{ float: 'right', paddingBottom: '5px' }}>
          <ReactHTMLTableToExcel
            ...
            className="btn-green"
            table="table-to-xls"
            filename="קובץ - בדיקת הארקות חשמל ומבנים"
            sheet="קובץ - בדיקת הארקות חשמל ומבנים"
                ...
                return (
                  <tr {...row.getRowProps()}>
                    {row.cells.map((cell) => {
                      ...
                      if (cell.column.id == "buildingName") {
                      ...
                      if (cell.column.id == "_id") {
                        return (
                          <td>
                            <a
                              href={
                                "http://localhost:8000/api/downloadFile?collec=groundingTests&id=" +
                                cell.value.toString()
                              }
                              target="_blank"
                            >
                    ...
                    <td role="cell">
                      ...
                      >
                        {" "}
                        <Link
                          to={`/GlobalGroundingTestsForm/${row.original._id}`}
                        >
