...
const SortingTable = (props) => {
  ...
  const getUnitDetailsByGdod = async () => {
    await axios.get(`http://localhost:8000/api/monthlySafetyCommitteesMonitoring/bygdod/${props.unitid}`)
      .then((response) => {
  ...
  const getUnitDetailsByHativa = async () => {
    await axios.get(`http://localhost:8000/api/monthlySafetyCommitteesMonitoring/byhativa/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const getUnitDetailsByOgda = async () => {
    await axios.get(`http://localhost:8000/api/monthlySafetyCommitteesMonitoring/byogda/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const getUnitDetailsByPikod = async () => {
    await axios.get(`http://localhost:8000/api/monthlySafetyCommitteesMonitoring/bypikod/${props.unitid}`)
      .then((response) => {
        setData(response.data);
        setOriginaldata(response.data);
          setIsdataloaded(true)
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/monthlySafetyCommitteesMonitoringDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/monthlySafetyCommitteesMonitoring/${tempData}`).then((response) => {
        init();
      })
        .catch((error) => {
          console.log(error);
      ...
      <>
        <div style={{ float: 'right', paddingBottom: '5px' }}>
          <ReactHTMLTableToExcel
            ...
            className="btn-green"
            table="table-to-xls"
            filename="קובץ - מעקב וועדות בטיחות חודשית"
            sheet="קובץ - מעקב וועדות בטיחות חודשית"
                  ...
                  <tr {...row.getRowProps()}>
                      ...
                      if (cell.column.id == "_id") {
                        return (
                          <td>
                            <a
                              href={
                                "http://localhost:8000/api/downloadFile?collec=monthlySafetyCommitteesMonitoring&id=" +
                                cell.value.toString()
                              }
                              target="_blank"
                            >
                    ...
                    <td role="cell">
                      ...
                      >
                        {" "}
                        <Link
                          to={`/GlobalMonthlySafetyCommitteesMonitoringForm/${row.original._id}`}
                        >
