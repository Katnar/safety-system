...
const SortingTable = (props) => {
  ...
  const Delete = (data) => {
    ...
    axios.post("http://localhost:8000/api/machinesAndEquipmentPeriodicInspectionsDelete", tempData).then((response) => {
      axios.delete(`http://localhost:8000/api/machinesAndEquipmentPeriodicInspections/${data._id}`).then((response) => {
        loadData();
              ...
              return (
                <tr {...row.getRowProps()}>
                  {row.cells.map((cell) => {
                    ...
                    if (cell.column.id == "meanName") {
                      return <td>{cell.value}</td>;
                    }
                    if (cell.column.id == "mstb") {
                      return <td>{cell.value}</td>;
                    }
                    ...
                    if (cell.column.id == "meanQuantity") {
                      return <td>{cell.value}</td>;
                    }
                  ...
                  <td role="cell">
                    ...
                    >
                      {" "}
                      <Link
                        to={`/machinesAndEquipmentPeriodicInspectionsForm/${row.original._id}`}
                      >
