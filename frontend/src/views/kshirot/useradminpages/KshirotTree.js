...
const KshirotTree = () => {
    ...
    function handleTreeClick(event) {
        ...
        if (event.type == "pikod") {
            history.push(`/editpikod/${idstr}`);
        }
        if (event.type == "ogda") {
          
            history.push(`/ogdaeditpage/${idstr}`);//idstr = _id
        }
        if (event.type == "hativa") {
            
            history.push(`/hativaeditpage/${idstr}`);//idstr = _id
        }
        if (event.type == "gdod") {
          
            var gdodskshirotid=event.gdodskshirotid;
            history.push(`/gdodeditpage/${idstr}`);//idstr = _id
        }
    }
