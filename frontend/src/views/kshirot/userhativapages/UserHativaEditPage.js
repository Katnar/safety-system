...
const UserHativaEditPage = ({ match }) => {
    ...
    <Container style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
        <h1 style={{ color: 'white', textAlign: 'center' }}>{hativa.name}</h1>
    </Container>
    ...
    return (
        <>
            ...
            <Container style={{ paddingTop: '20px' }}>
                <Card>
                    <CardHeader>
                        <CardTitle tag="h4" style={{ direction: 'rtl', textAlign: 'right' }}>מטא"ג חטיבתי נוכחי</CardTitle>{/*headline*/}
                    </CardHeader>
                    <CardBody style={{ direction: 'rtl', textAlign: 'right' }}>
                        {hativa.matag ? <>
                            <OneMatag id={hativa.matag} />
                            {user.kshirot === "1" ? null : <Link to={`/editmatag/${hativa._id}/${hativa.matag}`}><button className="btn btn-success">ערוך מטא"ג חטיבתי</button></Link> }
                            ...
                            <>
                             ...
                             <>
                                ...
                                <Link to={`/addmatag/${hativa._id}`}><button className="btn btn-success">הוסף מטא"ג חטיבתי</button></Link>
                                </>
                                }
                ...
                {hativa.mataghistory && hativa.mataghistory.length != 0 ?
                    <>
                        <OpenCardnew headline={'היסטורית מטא"ג'} content={<HativaMatagHistorySortingTable />} />
                    </> :
                    <>
