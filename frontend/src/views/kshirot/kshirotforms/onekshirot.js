...
const Onekshirot = ({ id }) => {
    ...
    const loadActivetipuls = () => {
        axios.get(`http://localhost:8000/api/kshirot/${id}`)
            .then(response => {
                setActivetipuls(response.data);
                CalculateKshirotSum(response.data);
            })
            .catch((error) => {
                console.log(error);
            })
    ...
    const card1tableheaders = [
        ...
        "שם",
        "מספר פלאפון"];
    const card1tabledata = activetipuls[0] ?
        [activetipuls[0].unit, activetipuls[0].commandername, activetipuls[0].timeinrole, activetipuls[0].name, activetipuls[0].phone]
        : null;
    ...
    const card2tabledata = activetipuls[0] ?
        [activetipuls[0].officers, activetipuls[0].officersmax]
        : null;
    ...
    const card3tableheaders = [
        ...
        "בקש מצבה",
        "תאריך רענון אחרון",
        "התאמת חלפים לצלם רישום מכין",
        "התאמת ערכות חלפים לצלמ",
        "קטלוגים",
    ];
    const card3tabledata = activetipuls[0] ?
        [activetipuls[0].teken, activetipuls[0].tekenmax, activetipuls[0].toolsbox, activetipuls[0].toolsboxmax, activetipuls[0].match, activetipuls[0].load, activetipuls[0].stash, activetipuls[0].hatak, activetipuls[0].bakash, activetipuls[0].bakashmax, activetipuls[0].lastrefreshdate, activetipuls[0].matchmahin, activetipuls[0].matchswap, activetipuls[0].catalogs,]
        : null;
    ...
    const card4tabledata = activetipuls[0] ?
        [activetipuls[0].carpiter, activetipuls[0].carpitermax, activetipuls[0].carhatap, activetipuls[0].carhatapmax, activetipuls[0].mobilitytools, activetipuls[0].carlahh, activetipuls[0].katkal, activetipuls[0].rioarrow, activetipuls[0].rioarrowmax, activetipuls[0].personalprotection]
        : null;
    ...
    const card5tabledata = activetipuls[0] ?
        [activetipuls[0].shiboz, activetipuls[0].shibozmax, activetipuls[0].drivers, activetipuls[0].driversmax, activetipuls[0].pkodotopara, activetipuls[0].tiom, activetipuls[0].commanderconf, activetipuls[0].pakalim, activetipuls[0].tikshorat, activetipuls[0].tikshoratmax, activetipuls[0].tkinot, activetipuls[0].tkinotmax, activetipuls[0].tikim, activetipuls[0].roleholders, activetipuls[0].roleholdersmax, activetipuls[0].boxcontent]
        : null;
    ...
    const card6tabledata = activetipuls[0] ?
        [activetipuls[0].trainingamount.slice(0, 10), activetipuls[0].trainingquality, activetipuls[0].battaliondrillamount, activetipuls[0].battaliondrillquality, activetipuls[0].kors, activetipuls[0].korsmax, activetipuls[0].nokavim, activetipuls[0].nokavimmax, activetipuls[0].tester, activetipuls[0].testermax, activetipuls[0].amountmhalaf, activetipuls[0].amountmhalafmax, activetipuls[0].amounthanafa, activetipuls[0].amounthanafamax]
        : null;
                    ...
                    <>
                        <Card>
                            <CardHeader style={{ direction: 'rtl' }}>
                                <CardTitle tag="h4" style={{ direction: 'rtl', textAlign: 'right' }}>ציון כללי: {kshirotsum.toString().slice(0, 4)}/100</CardTitle>{/*headline*/}
                            </CardHeader>

                            <CardBody style={{ direction: 'ltr', textAlign: 'right' }}>
                                {kshirotsum ?
                                    kshirotsum < 50 ?
                                        <Progress color="danger" value={kshirotsum} style={{height:'2rem'}}>{kshirotsum.toString().slice(0, 4)}%</Progress>
                                        : kshirotsum < 75 ?
                                            <Progress color="warning" value={kshirotsum} style={{height:'2rem'}}>{kshirotsum.toString().slice(0, 4)}%</Progress>
                                            : kshirotsum < 100 ?
                                                <Progress color="success" value={kshirotsum} style={{height:'2rem'}}>{kshirotsum.toString().slice(0, 4)}%</Progress>
                                                : null
                                    : null}
                            </CardBody>
                        </Card>
                        ...
                        <Card>
                            <CardHeader style={{ direction: 'rtl' }}>
                                <CardTitle tag="h4" style={{ direction: 'rtl', textAlign: 'right' }}>חוסן מנטלי: {activetipuls[0] ? activetipuls[0].mentality : null}</CardTitle>{/*headline*/}
                            </CardHeader>
                        </Card>
                    </>
                    : <>
